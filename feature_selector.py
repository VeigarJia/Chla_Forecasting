...
class FeatureSelector():
    ...
    def identify_zero_importance(self, n_iterations=10):
        """
        ...
        feature_importance_values_1 = np.zeros(len(feature_names))
        feature_importance_values_2 = np.zeros(len(feature_names))
        # feature_importance_values_3 = np.zeros(len(feature_names))

        print('Training Gradient Boosting Model\n')

        # Iterate through each fold
        for _ in range(n_iterations):
            model = xgb.XGBRegressor(random_state=161227)
            model.fit(features, labels)

            # Record the feature importances
            feature_importance_values_1 += model.feature_importances_ / n_iterations

        for _ in range(n_iterations):
            model = RandomForestRegressor(random_state=161227)
            model.fit(features, labels)
            # Record the feature importances
            feature_importance_values_2 += model.feature_importances_ / n_iterations

        # for _ in range(n_iterations):
        #     model = ExtraTreesRegressor(random_state=161227)
        #     model.fit(features, labels)
        #     # Record the feature importances
        #     feature_importance_values_3 += model.feature_importances_ / n_iterations

        feature_importance_values = np.mean([feature_importance_values_1, feature_importance_values_2], axis=0)
        # print(type(feature_importance_values_1))
        # print(feature_importance_values_1)
    ...
    def plot_feature_importances(self, plot_n=15, threshold=None):
        ...
        plt.tight_layout()
        plt.savefig('./pictures/Feature Importances.jpg', dpi=400)
        plt.show()
