...
others = pd.read_csv('Cluster_Results.csv').drop(['Station'], axis=1)
...
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=16122700)

print(sorted(y_train, reverse=True))
print(sorted(y_test, reverse=True))
...
y_trueDF.to_csv('y_trueDF_whole_nstation.csv', index=False)
y_true_testDF.to_csv('y_true_testDF_whole_nstation.csv', index=False)
